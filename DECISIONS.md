...
Phase 6: deep-model final deployment decision, 2026-09-25 (student)

LSTM, GRU, and CNN-LSTM will each receive a fresh final-window training run and will be retained as deployable serving artifacts. The final training window is the same registered window used for the LR/RF final artifacts: train plus validation data, with the h=6 eligibility/purge applied at the 2016-04-30 boundary. The test partition remains untouched and no test rows enter fitting, preprocessing, or artifact construction.

These are not repackaged walk-forward evaluation artifacts. Seed `42` will be used for all three final runs, but each is a new and distinct training run using the familiar seed value. It must not be described as the Phase 5 seed-42 model. The Phase 5 seed-42 models were separate 8-fold walk-forward fits trained on their respective smaller fold-specific training windows, evaluated, and discarded. These Phase 6 runs instead fit the full registered final training window and are retained as the deployed artifacts.

The final deep-model runs will reuse the registered Phase 4/5 `SequenceForecaster` configuration: sequence/window length `L=18`; hidden size `64`; one layer; dropout `0`; Adam learning rate `1e-3`; batch size `64`; Huber loss with `delta=40`; `50` epochs; no early stopping; PyTorch threads `4`; `16` sequence input columns consisting of `FEATURE_COLUMNS` excluding `hour_of_day` and `day_of_week`; and raw-Wh `Appliances` as the target. The implementation must reuse the existing configured/source-of-truth values for this Phase 4/5 configuration where they already exist rather than silently retyping them. If any value must be re-entered in the new final-training path, that is a potential configuration-drift risk and must be made explicit rather than assuming equivalence.

The earlier torch-serving amendment is scoped to the LR serving path and must remain accurate after these deployments. LR's own inference path does not perform PyTorch model computation. The earlier `skops`/`sklearn` behavior in which `torch` may become resident transitively during artifact-library initialization does not change that fact. Once LSTM, GRU, and CNN-LSTM are selectable through the read-only `/predict` endpoint, however, PyTorch inference genuinely and intentionally occurs at serving time for those specific `model_family` values. This is actual model computation, not merely a transitive import. For `model_families=["linear_regression"]`, the LR inference path performs no PyTorch model computation of consequence, although `torch` may still be resident in the process because of the previously documented `skops`/`sklearn` initialization behavior.

Each of the three final deep-model MLflow artifacts will record its model family, seed (`42`), epoch count (`50`), and code SHA using the existing Phase 6 MLflow tagging/schema convention used for the other final serving artifacts. This preserves reproducibility and makes it possible to answer which exact training run produced the artifact currently deployed.

LSTM, GRU, and CNN-LSTM are intentionally handled in one implementation pass because they share `SequenceForecaster`. The shared packaging/loading mechanism will be verified concretely once against LSTM before being applied to GRU and CNN-LSTM. The same successful mechanism will then be used for the other two families; three independent "it worked" reports are not treated as three separate validations of the shared mechanism.

This deployment decision is being recorded before the three final training runs and before the corresponding deployment artifacts exist. The choice of fresh final-window training, seed `42`, reused Phase 4/5 configuration, intentional PyTorch inference for the three sequence families, and shared packaging/loading approach is therefore not being changed retrospectively based on the eventual training results.
